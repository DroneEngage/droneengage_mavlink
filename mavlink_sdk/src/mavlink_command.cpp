...
void CMavlinkCommand::sendLongCommand(const uint16_t &command,
                                      ...
                                      const float &param3, const float &param4,
                                      const float &param5, const float &param6,
...
void CMavlinkCommand::sendIntCommand(const uint16_t &command,
                                     const uint8_t &frame, const float &param1,
                                     const float &param2, const float &param3,
                                     const float &param4, const int32_t &x,
                                     const int32_t &y, const float &z) const {
 ...
 */
void CMavlinkCommand::sendNative(
    const mavlink_message_t mavlink_message) const {
  m_mavlink_sdk.sendMavlinkMessage(mavlink_message);
  return;
}
...
void CMavlinkCommand::requestMessageEmit(const uint32_t message_id) const {
  ...
  // mavlink_message_t mavlink_message;
  // mavlink_msg_message_interval_encode(m_vehicle.getSysId(),mavlink_sdk.getCompId(),
  // &mavlink_message, &mavlink_message_interval);

  // mavlink_sdk.sendMavlinkMessage(mavlink_message);

  sendLongCommand(MAV_CMD_REQUEST_MESSAGE, false,
                  MAVLINK_MSG_ID_WIND, // use specified location . if 1 then use
                                       // current location.
                  0,                   // unused
                  0,                   // unused
                  0, 0, 0, 0);
...
void CMavlinkCommand::sendHeartBeatOfGCS(const uint8_t gcs_id) const {
  ...
  mavlink_heartbeat.autopilot = MAV_AUTOPILOT::MAV_AUTOPILOT_INVALID;
  ...
  mavlink_heartbeat.type = MAV_TYPE::MAV_TYPE_GCS;
...
void CMavlinkCommand::sendHeartBeatOfComponent(
    const uint8_t component_id) const {
  const mavlink_heartbeat_t mavlink_heartbeat = m_vehicle.getMsgHeartBeat();

  mavlink_message_t mavlink_message;
  mavlink_msg_heartbeat_encode(m_vehicle.getSysId(), component_id,
                               &mavlink_message, &mavlink_heartbeat);
...
void CMavlinkCommand::doArmDisarm(const bool &arm, const bool &force) const {
  ...
  sendLongCommand(MAV_CMD_COMPONENT_ARM_DISARM, true, (float)flagArm, forceArm);
 ...
 */
void CMavlinkCommand::doSetMode(const int &mode, const int &custom_mode,
                                const int &custom_sub_mode) const {

  sendLongCommand(MAV_CMD_DO_SET_MODE, true, (float)mode, (float)custom_mode,
...
void CMavlinkCommand::cmdTerminateFlight() const {
  sendLongCommand(MAV_CMD_DO_FLIGHTTERMINATION, false, 1);

  return;
}

/**
 * @brief change altitude by calling Guided mode with target point and new
 * altitude.
 * @details altitude will increase or decrease along the way to the target.
 * If you want instant change then you need to change the altitude in the
 * current point then move to target.
 * @param altitude
 */
 ...
 */
void CMavlinkCommand::gotoGuidedPoint(const double &latitude,
                                      const double &longitude,
                                      const double &relative_altitude) const {
 ...
 */
void CMavlinkCommand::setYawCondition(const double &target_angle,
                                      const double &turn_rate,
                                      const bool &is_clock_wise,
                                      const bool &is_relative) const {
  float direction = is_clock_wise ? 1.0f : -1.0f;
  float relative = is_relative ? 1.0f : 0.0f;

  sendLongCommand(MAV_CMD_CONDITION_YAW, false, target_angle, turn_rate,
                  direction, relative);
...
/**
 * @brief
 * @param speed_type Speed type (0=Airspeed, 1=Ground Speed, 2=Climb Speed,
 * 3=Descent Speed)
 ...
 */
void CMavlinkCommand::setNavigationSpeed(const int &speed_type,
                                         const double &speed,
                                         const double &throttle,
                                         const bool &is_relative) const {
  const int relative = is_relative == true ? 1 : 0;

  sendLongCommand(MAV_CMD_DO_CHANGE_SPEED, false, speed_type, speed, throttle,
                  relative);
 ...
 *
 */
void CMavlinkCommand::sendMissionAck(const uint8_t target_system,
                                     uint8_t target_component,
                                     const uint8_t result) const {
#ifdef DEBUG
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: sendMissionAck "
...
void CMavlinkCommand::getWayPointByNumber(const int &mission_number) const {
#ifdef DEBUG
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: getWayPointByNumber "
            << std::to_string(mission_number) << _NORMAL_CONSOLE_TEXT_
            << std::endl;
#endif
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_mission_request_int_encode(255, 190, &mavlink_message,
                                         &mission_request);
...
void CMavlinkCommand::clearWayPoints() const {
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_mission_clear_all_encode(GCS_SYSID, 190, &mavlink_message,
                                       &mission_clear);
...
/**
 * @brief Set the mission item with sequence number seq as current item. This
 * means that the MAV will continue to this mission item on the shortest path
...
void CMavlinkCommand::setCurrentMission(const int &mission_number) const {
#ifdef DEBUG
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: setCurrentMission "
            << std::to_string(mission_number) << _NORMAL_CONSOLE_TEXT_
            << std::endl;
#endif
  ...
  sendLongCommand(MAV_CMD_DO_SET_MISSION_CURRENT, true, (float)mission_number,
                  (float)1);
...
void CMavlinkCommand::requestMissionList() const {
#ifdef DEBUG
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: requestMissionList"
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
#endif
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_mission_request_list_encode(255, 190, &mavlink_message,
...
/**
 * @brief Set mission count required by ardupilot to start receiving messages
 ...
 * @param mission_type
 */
void CMavlinkCommand::setMissionCount(const int &mission_count,
                                      MAV_MISSION_TYPE mission_type) const {
#ifdef DEBUG
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: requestMissionList"
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_mission_count_encode(255, 190, &mavlink_message,
                                   &mission_count_msg);
...
void CMavlinkCommand::writeMission(
    std::map<int, mavlink_mission_item_int_t> mavlink_mission) const {
  mavlinksdk::CMavlinkWayPointManager::getInstance().saveWayPoints(
      mavlink_mission, MAV_MISSION_TYPE::MAV_MISSION_TYPE_MISSION);

  return;
}

void CMavlinkCommand::writeMissionItem(
    mavlink_mission_item_int_t mavlink_mission) const {
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_mission_item_int_encode(255, 190, &mavlink_message,
                                      &mavlink_mission);
...
void CMavlinkCommand::writeMissionItem(
    mavlink_mission_item_t mavlink_mission) const {
...
void CMavlinkCommand::requestParametersList() const {
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_param_request_list_encode(GCS_SYSID, 190, &mavlink_message,
                                        &mavlink_param);
...
/**
 * @brief
 * (MAVLink 2) Request all parameters of this component. All parameters should
 * be emitted in response as PARAM_EXT_VALUE. ! messages are not handled
 * correctly no by mavsdk module.
 * TODO: please fix
 */
void CMavlinkCommand::requestExtParametersList() const {
  ...
  // // Encode
  // mavlink_message_t mavlink_message;
  // mavlink_msg_param_ext_request_list_encode(GCS_SYSID,190, &mavlink_message,
  // &mavlink_param);
 ...
 *
 */
void CMavlinkCommand::writeParameter(const std::string &param_name,
                                     const double &value) const {
  // TODO to be implemented

  const std::map<std::string, mavlink_param_value_t> &parameters_list =
      mavlinksdk::CMavlinkParameterManager::getInstance().getParametersList();
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_param_set_encode(GCS_SYSID, 190, &mavlink_message,
                               &mavlink_param);
 ...
 * @brief
 * ! Not Tested
 * @param param_name name is saved in parameters_list =
 * mavlinksdk::CVehicle::getInstance().getParametersList()
 */
void CMavlinkCommand::readParameter(const std::string &param_name) const {
  ...
  mavlink_param.target_component = m_vehicle.getCompId();
  memcpy(mavlink_param.param_id, it->first.c_str(), 16);
  mavlink_param.param_index =
      -1; // Send -1 to use the param ID field as identifier (else the param id
          // will be ignored)

  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_param_request_read_encode(GCS_SYSID, 190, &mavlink_message,
                                        &mavlink_param);
...
void CMavlinkCommand::readParameterByIndex(const uint16_t &param_index) const {
  ...
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT
            << "DEBUG: parameter index:" << std::to_string(param_index)
            << std::endl;
#endif
...
void CMavlinkCommand::requestDataStream(MAV_DATA_STREAM stream_id) const {
  ...
  // Encode
  mavlink_message_t mavlink_message;
  mavlink_msg_request_data_stream_encode(GCS_SYSID, 190, &mavlink_message,
...
void CMavlinkCommand::setServo(const int &channel, const int &pwm) const {
  sendLongCommand(MAV_CMD_DO_SET_SERVO, false, (float)channel, (float)pwm);
 ...
 * 		    value 0 or UINT16_MAX meanse ignore & value (UINT16_MAX-1)
 * means release.
 */
void CMavlinkCommand::releaseRCChannels() const {
  ...
  mavlink_message_t mavlink_message;
  mavlink_msg_rc_channels_override_encode(GCS_SYSID, 190, &mavlink_message,
                                          &mavlink_rc_channels);
 ...
 * @param channel_length a number <= MAX_RC_CHANNELS (18) Minimum value is 1
 * @param ignoreValue is used to unify channels that you want to be ignored.
 */
void CMavlinkCommand::sendRCChannels2(const uint16_t channels[MAX_RC_CHANNELS],
                                     int channel_length, int ignoreValue) const {
if ((channel_length > MAX_RC_CHANNELS) ||
  ...
  */
  ...
  mavlink_rc_channels.chan1_raw = 
      channel_length >= 1 ? (channels[0] == ignoreValue ? 0 : channels[0]) : 0; // apply or ignore
  mavlink_rc_channels.chan2_raw =
      channel_length >= 2 ? (channels[1] == ignoreValue ? 0 : channels[1]) : 0; // apply or ignore
  mavlink_rc_channels.chan3_raw =
      channel_length >= 3 ? (channels[2] == ignoreValue ? 0 : channels[2]) : 0; // apply or ignore
  mavlink_rc_channels.chan4_raw =
      channel_length >= 4 ? (channels[3] == ignoreValue ? 0 : channels[3]) : 0; // apply or ignore
  mavlink_rc_channels.chan5_raw =
      channel_length >= 5 ? (channels[4] == ignoreValue ? 0 : channels[4]) : 0; // apply or ignore
  mavlink_rc_channels.chan6_raw =
      channel_length >= 6 ? (channels[5] == ignoreValue ? 0 : channels[5]) : 0; // apply or ignore
  mavlink_rc_channels.chan7_raw =
      channel_length >= 7 ? (channels[6] == ignoreValue ? 0 : channels[6]) : 0; // apply or ignore
  mavlink_rc_channels.chan8_raw =
      channel_length >= 8 ? (channels[7] == ignoreValue ? 0 : channels[7]) : 0; // apply or ignore
  mavlink_rc_channels.chan9_raw =
      channel_length >= 9 ? (channels[8] == ignoreValue ? UINT16_MAX - 1 : channels[8]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan10_raw =
      channel_length >= 10 ? (channels[9] == ignoreValue ? UINT16_MAX - 1 : channels[9]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan11_raw =
      channel_length >= 11 ? (channels[10] == ignoreValue ? UINT16_MAX - 1 : channels[10]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan12_raw =
      channel_length >= 12 ? (channels[11] == ignoreValue ? UINT16_MAX - 1 : channels[11]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan13_raw =
      channel_length >= 13 ? (channels[12] == ignoreValue ? UINT16_MAX - 1 : channels[12]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan14_raw =
      channel_length >= 14 ? (channels[13] == ignoreValue ? UINT16_MAX - 1 : channels[13]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan15_raw =
      channel_length >= 15 ? (channels[14] == ignoreValue ? UINT16_MAX - 1 : channels[14]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan16_raw =
      channel_length >= 16 ? (channels[15] == ignoreValue ? UINT16_MAX - 1 : channels[15]) : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan17_raw =
      channel_length >= 17 ? (channels[16] == ignoreValue ? UINT16_MAX - 1 : channels[16]) : UINT16_MAX - 1; // apply or ignore
    mavlink_rc_channels.chan18_raw =
      channel_length >= 18 ? (channels[17] == ignoreValue ? UINT16_MAX - 1 : channels[17]) : UINT16_MAX - 1; // apply or ignore
...
void CMavlinkCommand::sendRCChannels(const uint16_t channels[MAX_RC_CHANNELS],
  ...
  mavlink_rc_channels.chan1_raw =
      channel_length >= 1 ? channels[0] : 0; // apply or ignore
  mavlink_rc_channels.chan2_raw =
      channel_length >= 2 ? channels[1] : 0; // apply or ignore
  mavlink_rc_channels.chan3_raw =
      channel_length >= 3 ? channels[2] : 0; // apply or ignore
  mavlink_rc_channels.chan4_raw =
      channel_length >= 4 ? channels[3] : 0; // apply or ignore
  mavlink_rc_channels.chan5_raw =
      channel_length >= 5 ? channels[4] : 0; // apply or ignore
  mavlink_rc_channels.chan6_raw =
      channel_length >= 6 ? channels[5] : 0; // apply or ignore
  mavlink_rc_channels.chan7_raw =
      channel_length >= 7 ? channels[6] : 0; // apply or ignore
  mavlink_rc_channels.chan8_raw =
      channel_length >= 8 ? channels[7] : 0; // apply or ignore
  mavlink_rc_channels.chan9_raw =
      channel_length >= 9 ? channels[8] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan10_raw =
      channel_length >= 10 ? channels[9] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan11_raw =
      channel_length >= 11 ? channels[10] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan12_raw =
      channel_length >= 12 ? channels[11] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan13_raw =
      channel_length >= 13 ? channels[12] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan14_raw =
      channel_length >= 14 ? channels[13] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan15_raw =
      channel_length >= 15 ? channels[14] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan16_raw =
      channel_length >= 16 ? channels[15] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan17_raw =
      channel_length >= 17 ? channels[16] : UINT16_MAX - 1; // apply or ignore
  mavlink_rc_channels.chan18_raw =
      channel_length >= 18 ? channels[17] : UINT16_MAX - 1; // apply or ignore
 ...
 * @param yaw_rate
 */
void CMavlinkCommand::ctrlGuidedVelocityInLocalFrame(
    const float vx, const float vy, const float vz, const float yaw_rate,
    MAV_FRAME mav_frame) const {
  ...
  mavlink_set_position.type_mask =
  ...
  mavlink_msg_set_position_target_local_ned_encode(
      GCS_SYSID, m_vehicle.getCompId(), &mavlink_message,
      &mavlink_set_position);

  m_mavlink_sdk.sendMavlinkMessage(mavlink_message);

#ifdef DEBUG
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << std::to_string(mavlink_set_position.vx)
...
void CMavlinkCommand::gotoGuidedPoint_default(
  ...
  // 	mavlink_msg_position_target_global_int_encode(GCS_SYSID,190,
  // &mavlink_message, &msg);
  ...
  std::cout << "gotoGuidedPoint_default x:" << msg.x << " y:" << msg.y
            << " z:" << msg.z << std::endl;
}
 ...
 */
void CMavlinkCommand::gotoGuidedPoint_px4(
    const double &latitude, const double &longitude,
    const double &relative_altitude) const {
  const mavlink_global_position_int_t &mavlink_global_position_int =
      mavlinksdk::CVehicle::getInstance().getMsgGlobalPositionInt();
  std::cout << "mavlink_global_position_int.alt:"
            << std::to_string((mavlink_global_position_int.alt -
                               mavlink_global_position_int.relative_alt) /
                              1000)
            << "  alt:" << std::to_string(relative_altitude);
  sendIntCommand(
      MAV_CMD_DO_REPOSITION, MAV_FRAME_GLOBAL,
      -1.0f, // Ground speed, less than 0 (-1) for default
      MAV_DO_REPOSITION_FLAGS_CHANGE_MODE, // Loiter radius for planes. Positive
                                           // values only, direction is
                                           // controlled by Yaw value. A value
                                           // of zero or NaN is ignored.
      0.0f, // Loiter radius for planes. Positive values only, direction is
            // controlled by Yaw value. A value of zero or NaN is ignored.
      NAN,  // Yaw heading. NaN to use the current system yaw heading mode (e.g.
            // yaw towards next waypoint, yaw to home, etc.). For planes
            // indicates loiter direction (0: clockwise, 1: counter clockwise)
      (int)(latitude * 1e7),  // Latitude
      (int)(longitude * 1e7), // Longitude
      relative_altitude + (mavlink_global_position_int.alt -
                           mavlink_global_position_int.relative_alt) /
                              1000 // absolure altitude
  );
