...
public:
  ...
  void sendHeartBeatOfGCS(const uint8_t gcs_id) const;
  void sendHeartBeatOfComponent(const uint8_t component_id) const;
  void doSetMode(const int &mode, const int &custom_mode = 0,
  ...
  void gotoGuidedPoint(const double &latitude, const double &longitude,
                       const double &relative_altitude) const;
  void setYawCondition(const double &target_angle, const double &turn_rate,
                       const bool &is_clock_wise,
                       const bool &is_relative) const;
  void setNavigationSpeed(const int &speed_type, const double &speed,
  ...
  void setMissionCount(const int &mission_count,
                       MAV_MISSION_TYPE mission_type) const;
  void
  writeMission(std::map<int, mavlink_mission_item_int_t> mavlink_mission) const;
  ...
  void sendMissionAck(const uint8_t target_system,
...
protected:
  void gotoGuidedPoint_default(const double &latitude, const double &longitude,
                               const double &relative_altitude) const;
  void gotoGuidedPoint_px4(const double &latitude, const double &longitude,
                           const double &relative_altitude) const;
  void takeOff_default(const float &altitude) const;
  void takeOff_px4(const float &altitude) const;

  void sendIntCommand(const uint16_t &command, const uint8_t &frame,
                      const float &param1, const float &param2,
                      const float &param3, const float &param4,
                      const int32_t &x, const int32_t &y, const float &z) const;

  void sendLongCommand(const uint16_t &command,
                       const bool &confirmation = false,
                       const float &param1 = 0.0f, const float &param2 = 0.0f,
                       const float &param3 = 0.0f, const float &param4 = 0.0f,
                       const float &param5 = 0.0f, const float &param6 = 0.0f,
                       const float &param7 = 0.0f) const;

private:
  mavlinksdk::CMavlinkSDK &m_mavlink_sdk =
      mavlinksdk::CMavlinkSDK::getInstance();
