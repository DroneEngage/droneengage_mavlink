...
protected:
  ...
  void parseCommand(Json_de &andruav_message, const char *full_message,
                    const int &full_message_length, int messageType,
                    uint32_t permission) override;

private:
  de::fcb::CFCBMain &m_fcbMain = de::fcb::CFCBMain::getInstance();
  mission::CMissionManager &m_mission_manager =
      mission::CMissionManager::getInstance();
  mavlinksdk::CMavlinkSDK &m_mavlinksdk =
      mavlinksdk::CMavlinkSDK::getInstance();
  de::fcb::CFCBFacade &m_fcb_facade = de::fcb::CFCBFacade::getInstance();
  de::fcb::swarm::CSwarmManager &m_fcb_swarm_manager =
