...
// TODO: CREATE NEW COMMAND TYPE_AndruavMessage_ACTION to collect all Drone
// commands.
#define MESSAGE_FILTER                                                         \
  {                                                                            \
    TYPE_AndruavMessage_RemoteExecute, TYPE_AndruavMessage_FlightControl,      \
        ...
        TYPE_AndruavMessage_LightTelemetry, TYPE_AndruavMessage_ServoChannel,  \
        TYPE_AndruavMessage_Sync_EventFire, TYPE_AndruavMessage_MAVLINK,       \
        TYPE_AndruavMessage_SWARM_MAVLINK, TYPE_AndruavMessage_Make_Swarm,     \
...
// This is a timestamp used as instance unique number. if changed then
// communicator module knows module has restarted.
...
de::fcb::CFCBAndruavMessageParser &cAndruavResalaParser =
 ...
 *
 */
void _versionOnly(void) { std::cout << version_string << std::endl; }
...
void _usage(void) {
  _version();
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT "Options" << _NORMAL_CONSOLE_TEXT_
            << std::endl;
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT
      "\t--serial:          display serial number needed for registration"
            << _NORMAL_CONSOLE_TEXT_ << std::ends;
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT "\t                   -s "
            << _NORMAL_CONSOLE_TEXT_ << std::ends;
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT
      "\t--config:          name and path of configuration file. default "
      "[./de_mavlink.config.module.json]"
            << _NORMAL_CONSOLE_TEXT_ << std::ends;
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT "\t                   -c ./config.json"
            << _NORMAL_CONSOLE_TEXT_ << std::ends;
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT
      "\t--bconfig:          name and path of configuration file. default "
      "[./de_mavlink.config.module.local]"
            << _NORMAL_CONSOLE_TEXT_ << std::ends;
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT "\t                   -b ./config.local"
            << _NORMAL_CONSOLE_TEXT_ << std::ends;
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT "\t--version:         -v"
...
void _displaySerial(void) {
  _version();
  std::cout << std::endl
            << _INFO_CONSOLE_TEXT "Serial Number: " << _TEXT_BOLD_HIGHTLITED_
            << hardware_serial << _NORMAL_CONSOLE_TEXT_ << std::endl;
}
...
void onReceive(const char *message, int len, Json_de jMsg) {
  ...
  try {
    ...
    if (std::strcmp(jMsg[INTERMODULE_ROUTING_TYPE].get<std::string>().c_str(),
    ...
    //     {
    //         const std::string leader_sender =
    //         jMsg[ANDRUAV_PROTOCOL_SENDER].get<std::string>();
    //         de::fcb::swarm::CSwarmFollower& swarm_follower =
    //         de::fcb::swarm::CSwarmFollower::getInstance();
    //         swarm_follower.handle_leader_traffic(leader_sender, message,
    //         len);
...
void initLogger() {
  ...
  if ((jsonConfig.contains("logger_enabled") == false) ||
      (jsonConfig["logger_enabled"].get<bool>() == false)) {
    std::cout << _LOG_CONSOLE_BOLD_TEXT << "Logging is "
              << _ERROR_CONSOLE_BOLD_TEXT_ << "DISABLED"
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  std::cout << _LOG_CONSOLE_BOLD_TEXT << "Logging is "
            << _SUCCESS_CONSOLE_BOLD_TEXT_ << "ENABLED" << _NORMAL_CONSOLE_TEXT_
            << std::endl;

  auto t = std::time(nullptr);
  auto tm = *std::localtime(&t);
  std::ostringstream log_filename_final;
  log_filename_final << "./logs/log_" << std::put_time(&tm, "%d-%m-%Y_%H-%M-%S")
                     << ".log";
  mkdir("./logs/", 0777);

  std::cout << _LOG_CONSOLE_BOLD_TEXT
            << "Logging to folder: " << _INFO_CONSOLE_TEXT << log_filename
            << _INFO_CONSOLE_BOLD_TEXT << " filename:" << _INFO_CONSOLE_TEXT
            << log_filename_final.str() << _NORMAL_CONSOLE_TEXT_ << std::endl;
...
void initArguments(int argc, char *argv[]) {
  int opt;
  const struct GetOptLong::option options[] = {{"config", true, 0, 'c'},
                                               {"bconfig", true, 0, 'b'},
                                               {"serial", false, 0, 's'},
                                               {"version", false, 0, 'v'},
  ...
  GetOptLong gopt(argc, argv, "c:b:svoh", options);
...
void initDEModule(int argc, char *argv[]) {
  ...
  if (validateField(jsonConfig, "s2s_udp_packet_size",
    ...
    udp_chunk_size =
        std::stoi(jsonConfig["s2s_udp_packet_size"].get<std::string>());
  } else {
  ...
  // UDP Server
  cModule.init(
      jsonConfig["s2s_udp_target_ip"].get<std::string>().c_str(),
...
void init(int argc, char *argv[]) {
  ...
  std::cout << std::endl
            ...
            << "STARTING PLUGIN ===================" << _NORMAL_CONSOLE_TEXT_
            << std::endl;
  _version();

  std::cout << _LOG_CONSOLE_BOLD_TEXT
            << std::asctime(std::localtime(&instance_time_stamp))
            << instance_time_stamp << _INFO_CONSOLE_BOLD_TEXT
  ...
  if (jsonConfig.contains("event_fire_channel") &&
    ...
    de::fcb::mission::CMissionManager::getInstance().setEventChannel(
        jsonConfig["event_fire_channel"].get<int>(),
        jsonConfig["event_wait_channel"].get<int>());
  }
...
void uninit() {
  ...
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: Unint" << _NORMAL_CONSOLE_TEXT_
            << std::endl;
#endif
...
#ifdef DEBUG
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: Unint_after Stop"
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
#endif
...
void quit_handler(int sig) {
  std::cout << _INFO_CONSOLE_TEXT << std::endl
            << "TERMINATING AT USER REQUEST" << _NORMAL_CONSOLE_TEXT_
            << std::endl;
