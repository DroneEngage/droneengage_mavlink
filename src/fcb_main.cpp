 ...
 * @param len
 */
void CFCBMain::OnMessageReceived(const de::comm::CUDPProxy *udp_proxy,
  ...
  for (int i = 0; i < len; ++i) {
    uint8_t msgReceived = mavlink_parse_char(
        MAVLINK_CHANNEL_TELEMETRY, message[i], &mavlink_message, &status);
    if (msgReceived != 0) {
...
bool CFCBMain::connectToFCB() {
  ...
  case CONNECTION_TYPE_UDP:
    std::cout << _INFO_CONSOLE_TEXT << "UDP FCB-Connection Initializing"
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
    m_mavlink_sdk.connectUDP(
        (m_jsonConfig["fcb_connection_uri"])["ip"].get<std::string>().c_str(),
        (m_jsonConfig["fcb_connection_uri"])["port"].get<int>());
    return true;

  case CONNECTION_TYPE_TCP:
    std::cout << _INFO_CONSOLE_TEXT << "TCP FCB-Connection Initializing"
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
...
bool CFCBMain::init() {
  ...
  if (m_jsonConfig.contains(
          "default_optimization_level")) { // TODO: convert this to inline as
                                           // validatefield
    m_mavlink_optimizer.setOptimizationLevel(
        m_jsonConfig["default_optimization_level"].get<int>());
    m_mavlink_optimizer.reset_timestamps();
  }

  if (m_jsonConfig.contains("udp_proxy_enabled")) { // TODO: convert this to
                                                    // inline as validatefield
    m_enable_udp_telemetry_in_config =
  ...
  if (m_enable_udp_telemetry_in_config == true) {
    ...
    std::cout << _LOG_CONSOLE_BOLD_TEXT
              << "Udp Proxy: " << _SUCCESS_CONSOLE_BOLD_TEXT_ << "Enabled"
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
    PLOG(plog::info) << "Udp Proxy Enabled";

    u_int16_t udp_proxy_fixed_port =
        cLocalConfigFile.getNumericField("udp_proxy_fixed_port");
    if (udp_proxy_fixed_port ==
        0xffff) { // not found in local config ... not that local config has
                  // higher priority than the user config.

      if (validateField(
              m_jsonConfig, "udp_proxy_fixed_port",
              Json_de::value_t::number_unsigned)) { // TODO: convert this to
                                                    // inline as validatefield
    ...
    if (m_udp_telemetry_fixed_port != 0) {
      std::cout << _LOG_CONSOLE_BOLD_TEXT
                << "Udp Proxy fixed port is:" << _INFO_CONSOLE_TEXT
                << m_udp_telemetry_fixed_port << _NORMAL_CONSOLE_TEXT_
                << std::endl;
      PLOG(plog::info) << "Udp Proxy fixed port is:"
    ...
    std::cout << _LOG_CONSOLE_BOLD_TEXT
              << "Udp Proxy:" << _ERROR_CONSOLE_BOLD_TEXT_ << "Disabled"
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  // Initialize DE Pilot Manager
  de::fcb::depilot::CDEPilotManager::getInstance().init();
...
bool CFCBMain::uninit() {
  ...
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: ~CFCBMain  Scheduler Thread Off"
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
#endif

  
// Uninitialize DE Pilot Manager
  de::fcb::depilot::CDEPilotManager::getInstance().init();
  return true;
}

void CFCBMain::initVehicleChannelLimits() { readConfigParameters(); }
...
void CFCBMain::readConfigParameters() {
  ...
  if (m_jsonConfig.contains("rc_block_channel")) {
    m_andruav_vehicle_info.rc_block_channel =
        m_jsonConfig["rc_block_channel"].get<int>();

    if (m_andruav_vehicle_info.rc_block_channel >= 0) {
      std::cout << _LOG_CONSOLE_BOLD_TEXT << "RC Blocking is "
                << _ERROR_CONSOLE_BOLD_TEXT_ << "enabled "
                << _INFO_CONSOLE_BOLD_TEXT
                << "at channel: " << _INFO_CONSOLE_BOLD_TEXT
                << std::to_string(m_andruav_vehicle_info.rc_block_channel)
                << _ERROR_CONSOLE_BOLD_TEXT_ << " - IMPORTANT"
                << _NORMAL_CONSOLE_TEXT_ << std::endl;
    }
  } else {
  ...
  if (!m_jsonConfig.contains("rc_channels") ||
    ...
    std::cout << _INFO_CONSOLE_BOLD_TEXT << "RC Channels are not defined."
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
    std::cout << _INFO_CONSOLE_TEXT << "..... Assuming default values."
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
    std::cout << _INFO_CONSOLE_TEXT << "..... Please define values for safety."
              << _NORMAL_CONSOLE_TEXT_ << std::endl;

    m_rcmap_channels_info.use_smart_rc = true;
    memset(m_andruav_vehicle_info.rc_channels_enabled, true,
           RC_CHANNELS_MAX * sizeof(bool));
    memset(m_andruav_vehicle_info.rc_channels_reverse, false,
           RC_CHANNELS_MAX * sizeof(bool));
    memset(m_andruav_vehicle_info.rc_channels_min, (int16_t)1000,
           RC_CHANNELS_MAX * sizeof(int16_t));
    memset(m_andruav_vehicle_info.rc_channels_max, (int16_t)2000,
           RC_CHANNELS_MAX * sizeof(int16_t));
  ...
  if (rc_channels.contains("rc_smart_channels")) {
    if (rc_channels["rc_smart_channels"].contains("active")) {
      m_rcmap_channels_info.use_smart_rc =
          rc_channels["rc_smart_channels"]["active"].get<bool>();
    } else {
      m_rcmap_channels_info.use_smart_rc = true;
    }
    ...
    if ((m_rcmap_channels_info.use_smart_rc == true) &&
...
#ifdef DDEBUG
      std::cout << _LOG_CONSOLE_BOLD_TEXT << "RC Smart Channels are"
      ...
      if (rc_channels["rc_smart_channels"].contains("rc_channel_limits_max")) {
        ...
        m_andruav_vehicle_info
            .rc_channels_max[m_rcmap_channels_info.rcmap_roll] = *it++;
        m_andruav_vehicle_info
            .rc_channels_max[m_rcmap_channels_info.rcmap_pitch] = *it++;
        m_andruav_vehicle_info
            .rc_channels_max[m_rcmap_channels_info.rcmap_throttle] = *it++;
        m_andruav_vehicle_info
            .rc_channels_max[m_rcmap_channels_info.rcmap_yaw] = *it;
      }
      ...
      if (rc_channels["rc_smart_channels"].contains("rc_channel_limits_min")) {
        ...
        auto it = values.begin();
        m_andruav_vehicle_info
            .rc_channels_min[m_rcmap_channels_info.rcmap_roll] = *it++;
        m_andruav_vehicle_info
            .rc_channels_min[m_rcmap_channels_info.rcmap_pitch] = *it++;
        m_andruav_vehicle_info
            .rc_channels_min[m_rcmap_channels_info.rcmap_throttle] = *it++;
        m_andruav_vehicle_info
            .rc_channels_min[m_rcmap_channels_info.rcmap_yaw] = *it;
      }
    ...
    if (now - m_andruav_vehicle_info.rc_command_last_update_time >
      ...
      if (m_andruav_vehicle_info.flying_mode != VEHICLE_MODE_LAND) {
        ...
        uint32_t ardupilot_mode, ardupilot_custom_mode,
            ardupilot_custom_sub_mode;
        // TODO: VEHICLE_MODE_BRAKE mode should be generalized to work on
        // different autopilot types.
        CFCBModes::getArduPilotMode(
            VEHICLE_MODE_BRAKE, m_andruav_vehicle_info.vehicle_type,
  ...
  while (!m_exit_thread) {
    ...
    if (m_counter % 50 == 0) {
      ...
      if (m_vehicle.hasLidarAltitude()) {
        m_fcb_facade.sendDistanceSensorInfo(
            std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS),
            m_vehicle.getLidarAltitude());
      }
    }
    ...
    if (m_counter % 100 == 0) {
      ...
      m_fcb_facade.sendTerrainReport(
          std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS));
    }

    if (m_counter % 500 == 0) { // 5 sec

      // no need to send it faster as it is sent by an event  if there is an
      // important change in the values.
      m_fcb_facade.sendEKFInfo(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS));
      m_fcb_facade.sendVibrationInfo(
          std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS));
      m_fcb_facade.sendPowerInfo(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS));
      const bool fcb_connected =
          mavlinksdk::CVehicle::getInstance().isFCBConnected();
    ...
    if (m_counter % 1000 == 0) { // 10 sec
      ...
      const mavlink_mission_current_t mission_current =
          mavlinksdk::CMavlinkWayPointManager::getInstance()
              .getMissionCurrent();
...
void CFCBMain::OnHeartBeat_First(const mavlink_heartbeat_t &heartbeat) {
  m_andruav_vehicle_info.vehicle_type =
      CFCBModes::getAndruavVehicleType(heartbeat.type);
...
void CFCBMain::OnArmed(const bool &armed, const bool &ready_to_arm) {
  std::cout << std::endl
            << _SUCCESS_CONSOLE_BOLD_TEXT_ << "OnArmed/ReadyToArm"
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
...
void CFCBMain::OnFlying(const bool &is_flying) {
  std::cout << std::endl
            << _SUCCESS_CONSOLE_BOLD_TEXT_ << "OnFlying"
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  if (m_andruav_vehicle_info.is_flying != is_flying) {
    if (is_flying == true) {
      ...
      m_andruav_vehicle_info.flying_total_duration +=
          m_andruav_vehicle_info.flying_last_start_time;
...
void CFCBMain::OnHighLatencyModeChanged(const int &latency_mode) {
  ...
  if (latency_mode == 0) {
    std::cout << std::endl
              << _SUCCESS_CONSOLE_BOLD_TEXT_
              << "Latency Mode: " << _NORMAL_CONSOLE_TEXT_ << "Exit"
              << std::endl;
  } else {
    std::cout << std::endl
              << _SUCCESS_CONSOLE_BOLD_TEXT_
              << "Latency Mode: " << _NORMAL_CONSOLE_TEXT_ << "Enter"
              << std::endl;
  }
  return;
}
...
void CFCBMain::OnHighLatencyMessageReceived(const int &latency_mode) {
  // TODO: BE Done
  m_fcb_facade.sendHighLatencyInfo(
      std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS));
  return;
}

void CFCBMain::OnEKFStatusReportChanged(
...
void CFCBMain::OnADSBVechileReceived(
    const mavlink_adsb_vehicle_t &adsb_vehicle) {
  m_fcb_facade.sendADSBVehicleInfo(
      std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS));
  return;
}

void CFCBMain::OnDistanceSensorChanged(
    const mavlink_distance_sensor_t &distance_sensor) {
  m_fcb_facade.sendDistanceSensorInfo(
      std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS), distance_sensor);
  return;
}

void CFCBMain::OnStatusText(const std::uint8_t &severity,
                            const std::string &status) {
  try {
    if (!is_ascii((const signed char *)status.c_str(), status.length())) {
      std::cout << std::endl
                << _ERROR_CONSOLE_BOLD_TEXT_ << " -- OnStatusText -bad format-"
                << _NORMAL_CONSOLE_TEXT_ << status << std::endl;
      PLOG(plog::error)
          << " -- OnStatusText received a non UTF-8 message format:" << status;
      return;
    }

    std::cout << std::endl
              << _SUCCESS_CONSOLE_BOLD_TEXT_ << " -- OnStatusText "
              << _NORMAL_CONSOLE_TEXT_ << status << std::endl;

    m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS),
                                  0, ERROR_3DR, severity, status);
...
void CFCBMain::OnMissionACK(const int &result, const int &mission_type,
  ...
  m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS), 0,
                                ERROR_3DR, sevirity, result_msg);
  return;
}
...
void CFCBMain::OnWayPointReceived(
    const mavlink_mission_item_int_t &mission_item_int) {
  if (mission_item_int.mission_type == MAV_MISSION_TYPE_MISSION) {
    mission::CMissionItem *mission_item =
        mission::CMissionItemBuilder::getClassByMavlinkCMD(mission_item_int);
    mission_item->decodeMavlink(mission_item_int);
    de::fcb::mission::CMissionManager::getInstance()
        .getAndruavMission()
        .mission_items.insert(std::make_pair(
            mission_item_int.seq,
            std::unique_ptr<mission::CMissionItem>(mission_item)));
  }
 ...
 * if result is MAV_MISSION_RESULT::MAV_MISSION_ACCEPTED then mission has been
 ...
 * @param result_msg
 */
void CFCBMain::OnMissionSaveFinished(const int &result, const int &mission_type,
                                     const std::string &result_msg) {
  if (result == MAV_MISSION_RESULT::MAV_MISSION_ACCEPTED) {
    m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS),
                                  0, ERROR_TYPE_LO7ETTA7AKOM,
...
void CFCBMain::OnMissionCurrentChanged(
    const mavlink_mission_current_t &mission_current) {
  m_fcb_facade.sendMissionCurrent(std::string());

  // IMPORTANT: This is an internal only event and cannot be broadcasted to
  // communication-server. if you want to broadcast ato communication-server you
  // need to fire event from servo channels.

  // IMPORTANT: here we sent a local event of every mission item. this mission
  // item can be ANYTHING it is just a SYNC with other modules that has waiting
  // events linked to waypoints mission-items using. so instead of Filtering
  // message I have chosen to sent.

  // IMPORTANT: Ardupilot sends seq of mission items not mission commands.
  de::fcb::mission::CMissionManager::getInstance().handleMissionCurrentCount(
      mission_current);
}

void CFCBMain::OnACK(const int &acknowledged_cmd, const int &result,
                     const std::string &result_msg) {
  int sevirity;
  ...
  if (result != MAV_RESULT_ACCEPTED) {
    ...
    m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS),
                                  0, ERROR_3DR, sevirity, result_msg);
  }
 ...
 */
void CFCBMain::OnModeChanges(const uint32_t &custom_mode,
                             const int &firmware_type,
                             const MAV_AUTOPILOT &autopilot) {
  // vehicle type is determined here
  m_andruav_vehicle_info.vehicle_type =
      CFCBModes::getAndruavVehicleType(firmware_type);
  ...
  depilot::CDEPilotManager::getInstance().OnFlightModeChanged();
...
void CFCBMain::OnServoOutputRaw(
  ...
  de::fcb::mission::CMissionManager::getInstance().readWaitingEventFromFCB(
      servo_output_raw);
...
/**
 * @brief Called when parameter is recieved for first time or its value has
 * changed.
 * @details Forward parameters only when changed = true. As parameters are sent
 * in chunks to save bandwidth.
 ...
 */
void CFCBMain::OnParamReceived(const std::string &param_name,
  ...
  std::cout << __FILE__ << "." << __FUNCTION__ << " line:" << __LINE__ << "  "
            ...
            << " of " << std::to_string(param_message.param_count)
            << " type: " << std::to_string(param_message.param_type)
            << " value: " << std::to_string(param_message.param_value)
            << " changed: " << std::to_string(changed) << _NORMAL_CONSOLE_TEXT_
            << std::endl;
#endif
...
void CFCBMain::OnParamReceivedCompleted() {
  m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS), 0,
                                ERROR_TYPE_LO7ETTA7AKOM, NOTIFICATION_TYPE_INFO,
                                std::string("Parameters Loaded Successfully"));
 ...
 * @details called when connection between communication_pro and andruav server
 * changes.
 *
 * @param status
 */
void CFCBMain::OnConnectionStatusChangedWithAndruavServer(const int status) {
  ...
  case SOCKET_STATUS_CONNECTING: {
    PLOG(plog::info)
        << "Communicator Server Connection Status: SOCKET_STATUS_CONNECTING";
  } break;

  case SOCKET_STATUS_REGISTERED: {
    PLOG(plog::info)
        << "Communicator Server Connection Status: SOCKET_STATUS_REGISTERED";
...
/**
 * @brief This function get PWM signal from Andruav [0,1000] value and after
 * applying reverse, deadband & limites.
...
void CFCBMain::calculateChannels(const int16_t scaled_channels[MAX_RC_CHANNELS],
  ...
  for (int i = 0; i < MAX_RC_CHANNELS; ++i) {
    ...
    if ((scaled_channel == SKIP_RC_CHANNEL) ||
        (!m_andruav_vehicle_info.rc_channels_enabled[i])) {
...
/**
 * @brief disable rcchennel override and sends zero to all RC_CHANNELS_MAX
 * channels of ardupilot.
 * @callgraph
 */
void CFCBMain::releaseRemoteControl() {
  ...
  m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS), 0,
                                ERROR_RCCONTROL, NOTIFICATION_TYPE_WARNING,
                                std::string("RX Released."));
}

/**
 * @brief Set first 4 FOUR channels or user smart Channels based on RCMAP to
 * 1500 and release others.
 *
 */
void CFCBMain::centerRemoteControl() {
  ...
  if ((m_rcmap_channels_info.use_smart_rc) &&
    ...
    m_fcb_facade.sendErrorMessage(
        ...
        ERROR_TYPE_LO7ETTA7AKOM, NOTIFICATION_TYPE_ERROR,
  ...
  if ((m_rcmap_channels_info.use_smart_rc) &&
      (m_rcmap_channels_info.is_valid)) {
    m_andruav_vehicle_info.rc_channels[m_rcmap_channels_info.rcmap_pitch] =
        1500;
    m_andruav_vehicle_info.rc_channels[m_rcmap_channels_info.rcmap_roll] = 1500;
    m_andruav_vehicle_info.rc_channels[m_rcmap_channels_info.rcmap_throttle] =
        1500;
  ...
  m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS), 0,
                                ERROR_RCCONTROL, NOTIFICATION_TYPE_WARNING,
                                std::string("RX Centered and Locked"));
}
...
void CFCBMain::freezeRemoteControl() {
  ...
  if (m_andruav_vehicle_info.rc_sub_action ==
      RC_SUB_ACTION::RC_SUB_ACTION_FREEZE_CHANNELS)
    return;
  ...
  const mavlink_rc_channels_t &mavlink_rc_channels =
      mavlinksdk::CVehicle::getInstance().getRCChannels();
...
void CFCBMain::enableRemoteControl() {
  ...
  memset(m_andruav_vehicle_info.rc_channels, 0,
         RC_CHANNELS_MAX *
             sizeof(int16_t)); // zero values. RC channels will be sent later

  m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS), 0,
                                ERROR_RCCONTROL, NOTIFICATION_TYPE_WARNING,
                                std::string("RX Joystick Mode"));
}
...
void CFCBMain::enableRemoteControlGuided() {
  ...
  m_fcb_facade.sendErrorMessage(std::string(ANDRUAV_PROTOCOL_SENDER_ALL_GCS), 0,
                                ERROR_RCCONTROL, NOTIFICATION_TYPE_WARNING,
                                std::string("RX Joystick Guided Mode"));
...
void CFCBMain::adjustRemoteJoystickByMode(RC_SUB_ACTION rc_sub_action) {
  ...
  case RC_SUB_ACTION::RC_SUB_ACTION_JOYSTICK_CHANNELS:
  case RC_SUB_ACTION::RC_SUB_ACTION_JOYSTICK_CHANNELS_GUIDED: {
    if (m_andruav_vehicle_info.flying_mode ==
        VEHICLE_MODE_GUIDED) { // TODO CHECK FOR PLANES HERE
      // remote control in guided mode uses ctrlGuidedVelocityInLocalFrame for
      // control.
 ...
 */
void CFCBMain::updateRemoteControlChannels(
    const int16_t rc_channels[RC_CHANNELS_MAX]) {
  ...
  case RC_SUB_ACTION::RC_SUB_ACTION_JOYSTICK_CHANNELS: {
    ...
    if (m_rcmap_channels_info.use_smart_rc == true) {
      ...
      m_andruav_vehicle_info.rc_channels[m_rcmap_channels_info.rcmap_pitch] =
          rc_chammels_pwm[m_rcmap_channels_info.rcmap_pitch];
      m_andruav_vehicle_info.rc_channels[m_rcmap_channels_info.rcmap_roll] =
          rc_chammels_pwm[m_rcmap_channels_info.rcmap_roll];
      m_andruav_vehicle_info.rc_channels[m_rcmap_channels_info.rcmap_throttle] =
          rc_chammels_pwm[m_rcmap_channels_info.rcmap_throttle];
      m_andruav_vehicle_info.rc_channels[m_rcmap_channels_info.rcmap_yaw] =
          rc_chammels_pwm[m_rcmap_channels_info.rcmap_yaw];
    } else {
      // just copy channels.
  ...
  case RC_SUB_ACTION::RC_SUB_ACTION_JOYSTICK_CHANNELS_GUIDED: {
    ...
    if ((m_rcmap_channels_info.use_smart_rc) &&
      ...
      mavlinksdk::CMavlinkCommand::getInstance().ctrlGuidedVelocityInLocalFrame(
          ...
          (1500 - rc_chammels_pwm[3]) / 100.0f, MAV_FRAME_BODY_OFFSET_NED);
    }
  } break;
  }
}
...
void CFCBMain::update_rcmap_info() {
  mavlinksdk::CMavlinkParameterManager &parameter_manager =
      mavlinksdk::CMavlinkParameterManager::getInstance();
  ...
  case MAV_AUTOPILOT::MAV_AUTOPILOT_PX4: {
    ...
    mavlink_param_value_t rcmap =
        parameter_manager.getParameterByName("RC_MAP_PITCH");
    m_rcmap_channels_info.rcmap_pitch =
        (uint16_t)rcmap.param_value == 0 ? 2 : (uint16_t)rcmap.param_value - 1;

    rcmap = parameter_manager.getParameterByName("RC_MAP_ROLL");
    m_rcmap_channels_info.rcmap_roll =
        (uint16_t)rcmap.param_value == 0 ? 0 : (uint16_t)rcmap.param_value - 1;

    rcmap = parameter_manager.getParameterByName("RC_MAP_THROTTLE");
    m_rcmap_channels_info.rcmap_throttle =
        (uint16_t)rcmap.param_value == 0 ? 1 : (uint16_t)rcmap.param_value - 1;

    rcmap = parameter_manager.getParameterByName("RC_MAP_YAW");
    m_rcmap_channels_info.rcmap_yaw =
        (uint16_t)rcmap.param_value == 0 ? 3 : (uint16_t)rcmap.param_value - 1;
...
void CFCBMain::checkBlockedStatus() {
  ...
  channel +=
      (1 + m_andruav_vehicle_info.rc_block_channel); //+1 skip time stamp.

  const bool is_gcs_blocked = (*channel > BLOCKING_CHANNEL_HIGH_ACTIVE_PWM);

  if (m_jsonConfig.contains("read_only_mode")) {
    if (m_jsonConfig["read_only_mode"] ==
        true) { // if read only true the it is always read_only_mode = true
...
void CFCBMain::heartbeatCamera() {
   ...
   * acccessible from this mavlink module. so when clicking on Camera from
   * Mission Planner the command generated by Ardupilot it sent to mavlink
   * module that forwards it to DE_Camera. see: internalCommand_takeImage
   */
  mavlinksdk::CMavlinkCommand::getInstance().sendHeartBeatOfComponent(
      MAV_COMP_ID_CAMERA);
}
...
void CFCBMain::sendUdpProxyStatus(const std::string &target_party_id) {
  ...
  m_fcb_facade.sendUdpProxyStatus(std::string(), m_udp_proxy.enabled,
                                  m_udp_proxy.paused, m_udp_proxy.udp_ip2,
                                  m_udp_proxy.udp_port2,
                                  m_mavlink_optimizer.getOptimizationLevel());
  ;
}
...
void CFCBMain::pauseUDPProxy(const bool paused) { m_udp_proxy.paused = paused; }
...
void CFCBMain::updateDEPilotOperations() {
    // Delegate to the pilot manager
    de::fcb::depilot::CDEPilotManager::getInstance().updateOperations();
}
