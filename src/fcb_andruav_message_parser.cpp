...
void CFCBAndruavMessageParser::parseCommand(Json_de &andruav_message, const char *full_message, const int &full_message_length, int messageType, uint32_t permission)
{
    ...
    case TYPE_AndruavMessage_DoYAW:
    {
        ...
        // C : is_clock_wise
        // L : is_relative
        if ((!validateField(cmd, "A", Json_de::value_t::number_unsigned)) && (!validateField(cmd, "A", Json_de::value_t::number_integer)))
            return;
        if ((!validateField(cmd, "R", Json_de::value_t::number_unsigned)) && (!validateField(cmd, "R", Json_de::value_t::number_integer)))
            return;
    ...
    case TYPE_AndruavMessage_ChangeSpeed:
    {
        ...
        if (!validateField(cmd, "b", Json_de::value_t::boolean))
            return;
        if ((!validateField(cmd, "c", Json_de::value_t::number_float)) && (!validateField(cmd, "c", Json_de::value_t::number_integer)))
            return;
        if (!validateField(cmd, "d", Json_de::value_t::boolean))
            return;
    ...
    case TYPE_AndruavMessage_ServoChannel:
    {
        ...
        if ((!m_is_system) && ((permission & PERMISSION_ALLOW_GCS_MODES_SERVOS) != PERMISSION_ALLOW_GCS_MODES_SERVOS))
        {
    ...
    case TYPE_AndruavMessage_SWARM_MAVLINK:
    {
        ...
        if ((!m_is_system) && ((permission & PERMISSION_ALLOW_SWARM) != PERMISSION_ALLOW_SWARM))
        {
